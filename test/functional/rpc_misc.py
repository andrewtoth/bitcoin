...
class RpcMiscTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.restart_node(0, ["-txindex", "-blockfilterindex", "-coinstatsindex", "-txospenderindex", "-scriptpubkeyindex"])
        ...
        assert_equal(
            node.getindexinfo(),
            {
                ...
                "scriptpubkeyindex": values,
            }
        )
        # Specifying an index by name returns only the status of that index
        for i in {"txindex", "basic block filter index", "coinstatsindex", "txospenderindex", "scriptpubkeyindex"}:

...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-prune=<n>", strprintf("Reduce storage requirements by enabling pruning (deleting) of old blocks. This allows the pruneblockchain RPC to be called to delete specific blocks and enables automatic pruning of old blocks if a target size in MiB is provided. This mode is incompatible with a legacy (pre-hashing) -txindex or -txospenderindex. "
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.GetBoolArg("-txindex", DEFAULT_TXINDEX)) {
        ...
        if (chainman.m_blockman.IsPruneMode() && g_txindex->HasLegacyEntries()) {
    ...
    if (args.GetBoolArg("-txospenderindex", DEFAULT_TXOSPENDERINDEX)) {
        ...
        if (chainman.m_blockman.IsPruneMode() && g_txospenderindex->HasLegacyEntries()) {

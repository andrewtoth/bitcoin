...
public:
    ...
    bool BatchWrite(DirtyCoinsMap& data, const uint256&, bool erase) final
    {
        for (auto it = data.begin(); it != data.end(); it = erase ? data.erase(it) : std::next(it)) {
            if (it->second->coin.IsSpent() && (it->first.get().n % 5) != 4) {
                m_data.erase(it->first.get());
            } else if (erase) {
                m_data[it->first.get()] = std::move(it->second->coin);
            } else {
                m_data[it->first.get()] = it->second->coin;
            }
        }
        return true;
    }

...
static constexpr uint64_t MAX_SCRIPTPUBKEY_INDEX_CACHE{1_GiB};
...
CacheSizes CalculateCacheSizes(const ArgsManager& args, size_t n_indexes)
{
    ...
    // - scriptpubkeyindex (5%): serves getscripthistory prefix scans.
    ...
    index_sizes.scriptpubkey_index = std::min(total_cache * 5 / 100, args.GetBoolArg("-scriptpubkeyindex", DEFAULT_SCRIPTPUBKEYINDEX) ? MAX_SCRIPTPUBKEY_INDEX_CACHE : 0);
    if (n_indexes > 0) {
    ...
    total_cache -= index_sizes.scriptpubkey_index;
